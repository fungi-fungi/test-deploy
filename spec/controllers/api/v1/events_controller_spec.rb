...
RSpec.describe Api::V1::EventsController, :type => :controller do
  ...
  describe "Authorized request" do
    ...
    it 'gets empty array if there is no events' do
      get :index
    ...
    describe "to REST" do
      ...
      before(:each) do
        create_list(:event, WillPaginate.per_page)
      end

      it "returns events" do
        get :index
        expect(JSON.parse(response.body).size).to eq(Event.all.size)
      end

      describe 'pagination' do

        before(:each) do
          create_list(:event,  WillPaginate.per_page - 1)
        end
        ...
        it "returns correct amount of items on next page" do
          get :index, {page: '2'}
          expect(JSON.parse(response.body).size).to eq(Event.all.size - WillPaginate.per_page)
        end
