  ...
  describe "Authorized request" do
    ...
    describe "to REST" do
      ...
      describe '#create' do
        ...
        let(:entities) { created_entities.map { |entity| { sfid: entity.item.sfid, name: entity.item.name, amount: entity.i_m__amount__c, provider: 'rental' } } }
        let(:new_order_request) { OrderRequest.last }
        let(:new_request_bom) { RequestBom.last }
        let(:order_data) { { order: { entities: entities, event: event.sfid } } }
